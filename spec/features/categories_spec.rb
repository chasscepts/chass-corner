...
feature 'Category' do
  ...
  scenario 'article with highest vote to be shown on home page' do
    categories = new_categories
  ...
  scenario 'all articles in given category are listed on its page' do
    category = Category.create!(name: 'Category', priority: 1)
    articles = 6.times.map { |num| new_article(category, user1, "Article Number #{num}") }
